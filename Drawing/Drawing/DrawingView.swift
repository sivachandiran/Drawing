...
public let DRAWING_VERSION = "0.12.0"

/// Set yourself as the `drawingView`'s delegate to be notified when the active
/// tool changes.
 protocol DrawingViewDelegate: AnyObject {
    func drawingView(_ drawingView: DrawingView, didSwitchTo tool: DrawingTool)
    func drawingView(_ drawingView: DrawingView, didStartDragWith tool: DrawingTool)
    func drawingView(_ drawingView: DrawingView, didEndDragWith tool: DrawingTool)
    func drawingView(_ drawingView: DrawingView, didChangeStrokeColor strokeColor: UIColor?)
    func drawingView(_ drawingView: DrawingView, didChangeFillColor fillColor: UIColor?)
    func drawingView(_ drawingView: DrawingView, didChangeStrokeWidth strokeWidth: CGFloat)
    func drawingView(_ drawingView: DrawingView, didChangeFontName fontName: String)
    func drawingView(_ drawingView: DrawingView, didChangeFontSize fontSize: CGFloat)
}
...
class DrawingView: UIView {
    ...
    public var drawing: Drawing = Drawing(size: CGSize(width: 320, height: 320)) {
      didSet {
        ...
        if let tool = tool {
          delegate?.drawingView(self, didSwitchTo: tool)
        }
    ...
    /// Layer that backs `drawingView.selectionIndicatorView`. You may set this
    /// layer's properties to change its visual apparance. Its `path` and `frame`
    /// properties are managed by `drawingView`.
    ...
    public func set(tool: DrawingTool, shape: Shape? = nil) {
      ...
      DispatchQueue.main.async {
        ...
        self.delegate?.drawingView(self, didSwitchTo: tool)
      }
    }
    ...
    /// 1.0 if you are saving the image to a file
      ...
      return DrawingUtilities.renderImage(size: size, scale: scale) { (context: CGContext) -> Void in
        image?.draw(at: .zero)
    ...
    private func _didPan(sender: ImmediatePanGestureRecognizer) {
      ...
      let updateUncommittedShapeBuffers: () -> Void = {
        self.transientBufferWithShapeInProgress = DrawingUtilities.renderImage(size: self.drawing.size) {
      ...
      case .began:
        ...
        delegate?.drawingView(self, didStartDragWith: tool)
      ...
      case .ended:
        if sender.hasExceededTapThreshold {
          ...
          delegate?.drawingView(self, didEndDragWith: tool)
        } else {
    ...
    private func applySelectionViewState() {
      ...
      selectionIndicatorView.transform = ShapeTransform(
        translation: (
          ...
          offset + shape.transform.translation + CGPoint(x: -bounds.size.width * selectionIndicatorAnchorPointOffset.x, y: -bounds.size.height * selectionIndicatorAnchorPointOffset.y) + CGPoint(x: selectionBounds.size.width / 2, y: selectionBounds.size.height / 2)),
    ...
    private func redrawAbsolutelyEverything() {
        persistentBuffer = DrawingUtilities.renderImage(size: drawing.size) {
        for shape in self.drawing.shapes {
          shape.render(in: $0)
        }
      }
  ...
  // MARK: DrawingViewShapeUpdating implementation

  extension DrawingView: DrawingViewShapeUpdating {
  ...
  extension DrawingView: ToolSettingsDelegate {
    func toolSettings(
   ...
   */
  public protocol DrawingViewShapeUpdating: AnyObject {

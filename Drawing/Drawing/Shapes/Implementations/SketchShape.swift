...
public struct SketchLineSegment: Codable, Equatable {
...
class SketchShape: Shape, ShapeWithStrokeState, ShapeSelectable {
    private enum CodingKeys: String, CodingKey {
      case id, isFinished, strokeColor, start, strokeWidth, segments, isEraser, type, transform
    }

    public static let type: String = "Sketch"
    ...
    public var segments: [SketchLineSegment] = []
    ...
    public required init(from decoder: Decoder) throws {
      ...
      if type != SketchShape.type {
        throw DrawingDecodingError.wrongShapeTypeError
      }
      ...
      segments = try values.decode([SketchLineSegment].self, forKey: .segments)
    ...
    public func encode(to encoder: Encoder) throws {
      ...
      try container.encode(SketchShape.type, forKey: .type)
    ...
    public func add(segment: SketchLineSegment) {
    ...
    private func render(in context: CGContext, onlyLast: Bool = false) {
      ...
      var lastSegment: SketchLineSegment?

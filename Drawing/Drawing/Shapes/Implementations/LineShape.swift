  ...
  ShapeSelectable
{
  ...
  private func renderArrow(in context: CGContext) {
    ...
    let arrowOffset = CGPoint(x: angle, y: strokeWidth * 2)

    let startPoint = b + arrowOffset
    let point1 = b + CGPoint(x: angle + arcAmount / 2 + CGFloat.pi, y: radius) + arrowOffset
    let point2 = b + CGPoint(x: angle - arcAmount / 2 + CGFloat.pi, y: radius) + arrowOffset

...
public class Drawing: Codable {
  ...
  /**
   ...
   and you also want to use the `Codable` features of Drawing. It's simple:
  ...
  public required init(from decoder: Decoder) throws {
    ...
    while !shapeIter.isAtEnd {
      ...
      if shapes.count == countBefore {
        ...
        // move on.
        if Drawing.debugSerialization {
          throw DrawingDecodingError.unknownShapeTypeError(type)
        }
      }
    }
  }
...
public enum DrawingDecodingError: Error {
...
public class MultiDecoder<ResultType> {
  ...
  /// `DrawingDecodingError.wrongShapeTypeError` and
  ...
  public func decode<T: Shape>(_ type: T.Type) throws {
    do {
      ...
      return
    } catch DrawingDecodingError.wrongShapeTypeError {
      return
    }
  }
}
